...
function configure_pgsql {
  ...
  echo "docker exec ${_container_name} sh -c PGHOST=${_DB_HOST} PGPORT=${_DB_PORT} PGDATABASE=${_DB_ROOT_NAME} PGUSER=${_DB_ROOT_USER} PGPASSWORD=${_DB_ROOT_PASSWORD} psql --command=CREATE USER ${_DB_USER} WITH PASSWORD '${_DB_PASSWORD}';" > /tmp/test
